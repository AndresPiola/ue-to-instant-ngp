...
void Testbed::init_websocket_server()
{ 
	...
	try {
		...
		m_server.set_message_handler(bind([&](server* s, websocketpp::connection_hdl hdl, server::message_ptr  msg) {

			parse_camera_json_string(msg->get_payload());
			}, &m_server, ::_1, ::_2));
...
void Testbed::update_camera_from_direction(vec3 camera_direction )
{
	vec3 center =  { 0.5, 0.5, 0.5 }; 
	const vec3 old_cam_dir = camera_direction;
	camera_direction.x *= -nerf_scale;
	camera_direction.y *= -nerf_scale;
	camera_direction.z *= nerf_scale;

	vec3 camera_pos = center + camera_direction;
	vec3 dir = (camera_pos - m_camera[3]);
 	 
	m_camera[3] = camera_pos;
	vec3 cam_dir = normalize(center - m_camera[3]);
	set_view_dir(cam_dir);
	m_camera[3] = camera_pos;
	vec3 translation = vec3{ 0.0f, 0.0f, 0.0f }; 
	translate_camera(translation, mat3(m_camera));
//	std::string msg_log = "update_camera_from_direction=>" + v3_to_string(old_cam_dir);
	//logger(msg_log);
}
bool  Testbed::parse_camera_json_string(std::string json_string)
{
	try
	{ 
		...
		if (document.HasMember("rpc"))
		{
			...
			const std::string rpc = document["rpc"].GetString();
			if (rpc == "load_nerf")
			{
				const std::string file_path =	document["file_path"].GetString();
		...
		if (!bInitialized)
		{
			nerf_scale = document["nerf_scale"].GetDouble();
			double  new_fov = document["fov"].GetDouble();
			if (new_fov != fov())
			{
				set_fov(new_fov);
			}
		...
		auto camera_array = document["direction"].GetArray();
		vec3 camera_direction = vec3(camera_array[0].GetFloat(), camera_array[1].GetFloat(), camera_array[2].GetFloat());
		update_camera_from_direction(camera_direction);
...
void Testbed::send_texture_as_bytearray( const ivec2& resolution/*, GLint rgba_texture, GLint framebuffer*/)
{
	...
	if (width == 0 || height == 0)
	{
	 ...
	 bool front_buffer = true;
		...
		for (auto it : m_connections) {
				m_server.interrupt(it);
				m_server.send(it, buf.data(), buf.size(), websocketpp::frame::opcode::binary);
				 
		} 
			
			if (bWritePng || !bUseHttp)
			{ 
				std::remove("test.png");
				cv::imwrite("test.png", img, { cv::ImwriteFlags::IMWRITE_PNG_COMPRESSION, 9 });
...
void Testbed::imgui() {
	...
	if (ImGui::CollapsingHeader("Camera", ImGuiTreeNodeFlags_DefaultOpen)) {
		...
		ImGui::Checkbox("Use Http", &bUseHttp);
		ImGui::Checkbox("Save Png", &bWritePng);
		 	ImGui::SameLine();
...
void Testbed::left_mouse_click(bool bShift,  vec2 rel)
{
	ivec2 mouse = { ImGui::GetMousePos().x, ImGui::GetMousePos().y };
	if (bShift) {
		...
		//printf("from mouse %f %f  ", rel.x, rel.y );
...
bool Testbed::frame() {
#ifdef NGP_GUI
	//read_camera_json_file();
	send_texture_as_bytearray();
	...
	if (  (std::chrono::steady_clock::now() - m_last_gui_draw_time_point) > 25ms) {
