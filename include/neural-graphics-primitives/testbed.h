...
public:
	...
	bool bWritePng = false;
	bool bUseHttp = true;
    vec3 previous_camera_direction  ; 
	double nerf_scale = 3;
	std::vector<uchar> buf; 
	mat4x4 previous_transform_matrix;
 	 void left_mouse_click(bool bShift, vec2 rel); 
	void update_camera_from_direction(vec3 camera_direction );
	 
	void init_websocket_server();  
	bool parse_camera_json_string(std::string json_string);
	void send_texture_as_bytearray(const ivec2& resolution/*, GLint rgba_texture, GLint framebuffer*/);
	void send_texture_as_bytearray()
	{
		send_texture_as_bytearray(ivec2(800, 600));
	} 
	...
	std::string v3_to_string(vec3 vector3)
	{
		std::stringstream ss;
	...
	void read_camera_json_file()
	{
		using namespace rapidjson;
		 
		try
		{
			 std::ifstream file("camera_data.json");
			 // Read the entire file into a string
			  std::string json_string((std::istreambuf_iterator<char>(file)),
				 std::istreambuf_iterator<char>());
			  parse_camera_json_string(json_string);
	...
	std::string serializeTimePoint(const std::chrono::system_clock::time_point& time, const std::string& format= "UTC: %Y-%m-%d %H:%M:%S")
	{
	...
	void print_m4x4(mat4x4 transform_matrix)
	{
		int m = 4, n = 4;
		for (int i = 0; i < m; i++)
			for (int j = 0; j < n; j++)        
	...
	mat4x4 UEMoveTransform(mat4x4  m , float x,float y,float z) {
		m[0][3] = m[0][3] + x;
		m[1][3] = m[1][3] + y;
		m[2][3] = m[2][3] + z;
		return m;
		}
	mat4x4 UEScaleTransform(mat4x4  m, float  s) {
		m[0][3] = m[0][3] * s;
		m[1][3] = m[1][3] * s;
		m[2][3] = m[2][3] * s;
		return m;
	}
	mat4x4 UE_to_iNGP(mat4x4  m  ) {
		float temp1 = m[0][0];
		float temp2 = m[0][1];
		m[0][0] = m[0][1];
		m[0][1] = m[0][2];
	...
	int width,height; 
	web::http::experimental::listener::http_listener* listenServer;  
	...
	struct ImGuiVars {
		...
		char snapshot_path[MAX_PATH_LEN] = "D://Projects/instant-ngp/data/uechair.ingp";
